...
start_server {tags {"modules usercall network"}} {
    ...
    test {test module check regular kv command without user/acl} {
    ...
    test {test module check regular kv command with user} {
    ...
    test {test module check regular kv command with user and acl} {
        assert_equal [r set x 5] OK
    ...
    # call with user with acl set on it, but with testing the acl in rm_call (for cmd itself)
    test {test module check regular kv command with user and acl from blocked background thread} {
        assert_equal [r set x 5] OK
