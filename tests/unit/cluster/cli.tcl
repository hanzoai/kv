# Primitive tests on cluster-enabled server using kv-cli
...
start_multiple_servers 3 [list overrides $base_conf] {
    test {Create 1 node cluster} {
        exec $::KV_CLI_BIN --cluster-yes --cluster create \
...
start_multiple_servers 3 [list overrides $base_conf] {
    ...
    set node3_rd [kv_deferring_client -2]
    ...
    test "Perform a Resharding (with ASM $use_atomic_slot_migration)" {
        if {$use_atomic_slot_migration} {
            exec $::KV_CLI_BIN --cluster-yes --cluster reshard 127.0.0.1:[srv -2 port] \
    ...
    test "Wait for cluster to be stable (with ASM $use_atomic_slot_migration)" {
        ...
        wait_for_condition 1000 50 {
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv 0 port]}] == 0 &&
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok}
        } else {
    ...
    set node1_rd [kv_deferring_client 0]
    ...
    test "use previous hostip in \"cluster-preferred-endpoint-type unknown-endpoint\" mode (with ASM $use_atomic_slot_migration)" {
        ...
        # when kv-cli not in cluster mode, return MOVE with empty host
        ...
        assert_equal "[exec $::KV_CLI_BIN -h 127.0.0.1 -p [srv 0 port] -c set foo bar]" {OK}
        assert_match "[exec $::KV_CLI_BIN -h 127.0.0.1 -p [srv 0 port] -c get foo]" {bar}
...
# Test kv-cli -- cluster create, add-node, call.
# Test that functions are propagated on add-node
start_multiple_servers 5 [list overrides $base_conf] {

    set node4_rd [kv_client -3]
    set node5_rd [kv_client -4]

    test {Functions are added to new node on kv-cli cluster add-node} {
        exec $::KV_CLI_BIN --cluster-yes --cluster create \
                           127.0.0.1:[srv 0 port] \
                           127.0.0.1:[srv -1 port] \
                           127.0.0.1:[srv -2 port]
        ...
        exec $::KV_CLI_BIN --cluster-yes --cluster call 127.0.0.1:[srv 0 port] \
                           FUNCTION LOAD {#!lua name=TEST
                               server.register_function('test', function() return 'hello' end)
                           }

        # adding node to the cluster
        exec $::KV_CLI_BIN --cluster-yes --cluster add-node \
...
# Test kv-cli --cluster create, add-node.
# Test that one slot can be migrated to and then away from the new node.
test {Migrate the last slot away from a node using kv-cli} {
    start_multiple_servers 4 [list overrides $base_conf] {
        ...
        # Insert some data
        assert_equal OK [exec $::KV_CLI_BIN -c -p [srv 0 port] SET foo bar]
        set slot [exec $::KV_CLI_BIN -c -p [srv 0 port] CLUSTER KEYSLOT foo]
        ...
        wait_for_condition 1000 50 {
            ...
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [catch {exec $::KV_CLI_BIN --cluster check 127.0.0.1:[srv -3 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok} &&
            [CI 3 cluster_state] eq {ok}
        ...
        set owner_r [kv $owner_host $owner_port 0 $::tls]
        ...
        # Move the only slot back to original node using kv-cli
        exec $::KV_CLI_BIN --cluster reshard 127.0.0.1:[srv -3 port] \
...
# Test kv-cli --cluster create, add-node with cluster-port.
...
start_server [list overrides [list cluster-enabled yes cluster-node-timeout 1 cluster-port [find_available_port $::baseport $::portcount]]] {
    ...
    test "kv-cli -4 --cluster create using $ip_or_localhost with cluster-port" {
        exec $::KV_CLI_BIN -4 --cluster-yes --cluster create \
    ...
    test "kv-cli -4 --cluster add-node using $ip_or_localhost with cluster-port" {
        # Adding node to the cluster (without cluster-port)
        exec $::KV_CLI_BIN -4 --cluster-yes --cluster add-node \
...
start_multiple_servers 3 [list overrides $base_conf] {
    ...
    test "Perform a Multi-database Resharding (with ASM $use_atomic_slot_migration)" {
        ...
        for {set i 0} {$i < 4} {incr i} {
            if {$use_atomic_slot_migration} {
                exec $::KV_CLI_BIN --cluster-yes --cluster reshard 127.0.0.1:[srv 0 port] \
