...
extern int VM_CallArgvAbort(KVModuleCallArgvBlockedHandle *handle);
extern int VM_ReplyRaw(KVModuleCtx *ctx, const char *proto, size_t proto_len);
#define KVModule_CallArgv VM_CallArgv
#define KVModule_CallArgvAbort VM_CallArgvAbort
...
struct CallArgvAsyncContext {
    int is_async;
    KVModuleCallArgvBlockedHandle *abort_handle;
    KVModuleBlockedClient *bc;
};

static int call_argv_raw_resp_handler(void *ctx, KVModuleCtx *mctx, const char *proto, size_t proto_len) {
    UNUSED(proto_len);

    struct CallArgvAsyncContext *actx = (struct CallArgvAsyncContext *)(void *)ctx;

    if (actx->bc) {
        KVModuleCtx *bctx = KVModule_GetThreadSafeContext(actx->bc);
        KVModule_ReplyRaw(bctx, proto, proto_len);
        KVModule_FreeThreadSafeContext(bctx);
        KVModule_UnblockClient(actx->bc, KVModule_BlockClientGetPrivateData(actx->bc));
    } else if (!actx->is_async) {
        KVModule_ReplyRaw(mctx, proto, proto_len);
    } else {
...
static void do_vm_call_argv_async_disconnect(KVModuleCtx *ctx, struct KVModuleBlockedClient *bc) {
    UNUSED(ctx);
    struct CallArgvAsyncContext *actx = KVModule_BlockClientGetPrivateData(bc);
    KVModule_CallArgvAbort(actx->abort_handle);
    KVModule_AbortBlock(bc);
    actx->bc = NULL;
...
static void call_argv_deferred_reply_handle(void *ctx, KVModuleCtx *mctx,
                                             KVModuleCallArgvBlockedHandle *abort_handle) {
    struct CallArgvAsyncContext *actx = (struct CallArgvAsyncContext *)(void *)ctx;
    actx->is_async = 1;
    ...
    if (!actx->bc) {
        KVModuleBlockedClient *bc = KVModule_BlockClient(mctx, NULL, NULL, NULL, 0);
        KVModule_SetDisconnectCallback(bc, do_vm_call_argv_async_disconnect);
        KVModule_BlockClientSetPrivateData(bc, ctx);
        actx->bc = bc;
    }
}

int do_vm_call_argv_async_aux(KVModuleCtx *ctx, KVModuleString **argv, int argc, int flags, KVModuleBlockedClient *bc) {
    if(argc < 1){
        return KVModule_WrongArity(ctx);
    }

    struct CallArgvAsyncContext *actx = KVModule_Alloc(sizeof(*actx));
    *actx = (struct CallArgvAsyncContext){
    ...
    KVModuleReplyHandlers resp_handlers = {
     ...
     KVModule_CallArgv(ctx, argv, (size_t)argc, flags, &resp_handlers, (void *)actx);
     return KVMODULE_OK;
}
 ...
 * passing the rest of the arguments to the command invocation.
 * If the command got blocked, blocks the client and unblock it when the command gets unblocked,
 * this allows check the KVMODULE_CALL_ARGV_ALLOW_BLOCK flag to VM_CallArgv.
 */
int do_vm_call_argv_async(KVModuleCtx *ctx, KVModuleString **argv, int argc) {
    int flags = KVMODULE_CALL_ARGV_ERRORS_AS_REPLIES;
...
void do_vm_call_argv_async_script_mode_deferred_reply_handle(void *ctx, KVModuleCtx *mctx,
                                                             KVModuleCallArgvBlockedHandle *abort_handle) {
    UNUSED(ctx);
    UNUSED(abort_handle);
    KVModule_ReplyWithSimpleString(mctx, "Blocked");
}

int do_vm_call_argv_async_fire_and_forget(KVModuleCtx *ctx, KVModuleString **argv, int argc){
    if(argc < 2){
        return KVModule_WrongArity(ctx);
    }

    int flags = KVMODULE_CALL_ARGV_ERRORS_AS_REPLIES |
        KVMODULE_CALL_ARGV_REPLICATE |
        KVMODULE_CALL_ARGV_ALLOW_BLOCK;
    ...
    KVModule_CallArgv(ctx, argv + 1, (size_t)argc - 1, flags, &resp_handlers, NULL);
    return KVMODULE_OK;
}
...
void *send_call_argv_async_reply(void *arg) {
    ...
    KVModuleCtx *bctx = KVModule_GetThreadSafeContext(ta_call_ctx->bc);
    KVModule_ReplyRaw(bctx, ta_call_ctx->proto, ta_call_ctx->proto_len);
    KVModule_FreeThreadSafeContext(bctx);
    KVModule_UnblockClient(ta_call_ctx->bc, KVModule_BlockClientGetPrivateData(ta_call_ctx->bc));
    KVModule_Free(ta_call_ctx->proto);
    KVModule_Free(ta_call_ctx);
    return NULL;
}

static int call_argv_async_on_thread_available_handler(void *ctx, KVModuleCtx *mctx, const char *proto, size_t proto_len) {
    UNUSED(proto_len);

    struct CallArgvAsyncContext *actx = (struct CallArgvAsyncContext *)(void *)ctx;

    if (actx->bc) {
        ThreadedAsyncCallArgvCtx *ta_call_ctx = KVModule_Alloc(sizeof(*ta_call_ctx));
        ta_call_ctx->bc = actx->bc;
        ta_call_ctx->proto = KVModule_Alloc(proto_len);
...
int do_vm_call_argv_async_on_thread(KVModuleCtx *ctx, KVModuleString **argv, int argc) {
    if(argc < 2){
        return KVModule_WrongArity(ctx);
    }
    ...
    *actx = (struct CallArgvAsyncContext){
    ...
    KVModule_CallArgv(ctx, argv + 1, (size_t)argc - 1, flags, &resp_handlers, actx);
    return KVMODULE_OK;
}
...
int wait_and_call_argv_raw_resp_handler(void *ctx, KVModuleCtx *mctx, const char *proto, size_t proto_len) {
    struct WaitAndCallArgvAsyncContext *actx = (struct WaitAndCallArgvAsyncContext *)ctx;

    if (actx->is_async) {
        int flags = KVMODULE_CALL_ARGV_ALLOW_BLOCK |
            KVMODULE_CALL_ARGV_ERRORS_AS_REPLIES |
            KVMODULE_CALL_ARGV_REPLICATE;
...
void wait_and_call_argv_deferred_reply_handle(void *ctx, KVModuleCtx *mctx,
                                               KVModuleCallArgvBlockedHandle *abort_handle) {
    UNUSED(abort_handle);
    struct WaitAndCallArgvAsyncContext *actx = (struct WaitAndCallArgvAsyncContext *)ctx;
    KVModuleBlockedClient *bc = KVModule_BlockClient(mctx, NULL, NULL, NULL, 0);
...
static void forward_call_to_unblock_handler(KVModuleCtx *ctx,
                                            KVModuleString **argv,
                                            int argc,
                                            KVModuleString **fw_args,
                                            int fw_argc,
                                            int flags,
                                            const KVModuleReplyHandlers *resp_handlers) {

    struct WaitAndCallArgvAsyncContext *actx = KVModule_Alloc(sizeof(struct WaitAndCallArgvAsyncContext));
    *actx = (struct WaitAndCallArgvAsyncContext){
    ...
    KVModule_CallArgv(ctx, argv, argc, flags, resp_handlers, actx);
}

int wait_and_do_vm_call_argv_async(KVModuleCtx *ctx, KVModuleString **argv, int argc) {
    if(argc < 2){
        return KVModule_WrongArity(ctx);
    }

    KVModuleString *wait_argv[3];
    wait_argv[0] = KVModule_CreateString(NULL, "wait", 4);
    wait_argv[1] = KVModule_CreateString(NULL, "1", 1);
    wait_argv[2] = KVModule_CreateString(NULL, "0", 1);
    ...
    forward_call_to_unblock_handler(
        ctx, (KVModuleString **)wait_argv, 3, argv + 1, argc - 1, flags, &resp_handlers);

    KVModule_FreeString(NULL, wait_argv[0]);
    KVModule_FreeString(NULL, wait_argv[1]);
    KVModule_FreeString(NULL, wait_argv[2]);

    return KVMODULE_OK;
}

static int blpop_and_set_multiple_keys_on_available(void *ctx, KVModuleCtx *mctx, const char *proto, size_t proto_len) {
    struct WaitAndCallArgvAsyncContext *actx = (struct WaitAndCallArgvAsyncContext *)(void *)ctx;

    if (actx->is_async) {
        int flags = KVMODULE_CALL_ARGV_REPLICATE;

        KVModuleString *set_argv[3];
        set_argv[0] = KVModule_CreateString(NULL, "set", 3);

        for (int i = 0 ; i < actx->argc ; i += 2) {
            set_argv[1] = actx->argv[i];
            set_argv[2] = actx->argv[i + 1];
            KVModule_CallArgv(mctx, set_argv, 3, flags, NULL, NULL);
        }

        KVModule_FreeString(NULL, set_argv[0]);

        KVModuleCtx *bctx = KVModule_GetThreadSafeContext(actx->bc);
        KVModule_ReplyWithSimpleString(bctx, "OK");
        KVModule_FreeThreadSafeContext(bctx);
        KVModule_UnblockClient(actx->bc, NULL);
    ...
    for (int i = 0 ; i < actx->argc ; i++) {
        KVModule_FreeString(NULL, actx->argv[i]);
    }
    KVModule_Free(actx->argv);
    KVModule_Free(actx);
    return 0; /* stop parsing */
}

int blpop_and_set_multiple_keys_with_vm_call_argv(KVModuleCtx *ctx, KVModuleString **argv, int argc) {
    if(argc < 2){
        return KVModule_WrongArity(ctx);
    }

    KVModuleString *blpop_argv[3];
    blpop_argv[0] = KVModule_CreateString(NULL, "blpop", 5);
    blpop_argv[1] = argv[1];
    blpop_argv[2] = KVModule_CreateString(NULL, "0", 1);
    ...
    KVModuleString **fw_argv = KVModule_Alloc((argc - 2) * sizeof(KVModuleString*));
    for (int i = 0 ; i < argc - 2 ; ++i) {
        fw_argv[i] = KVModule_HoldString(NULL, argv[i + 2]);
    }

    forward_call_to_unblock_handler(
        ctx, (KVModuleString **)blpop_argv, 3, fw_argv, argc - 2, flags, &resp_handlers);

    KVModule_FreeString(NULL, blpop_argv[0]);
    KVModule_FreeString(NULL, blpop_argv[2]);
