...
set ::kv_base_port 30000
set ::kv_port_count 1024
set ::host "127.0.0.1"
...
if {[catch {cd tmp}]} {
    ...
    puts "Please run this test from the KV source root."
    exit 1
}
...
proc exec_instance {type dirname cfgfile} {
    if {$type eq "kv"} {
        set program_path $::KV_SERVER_BIN
    } elseif {$type eq "sentinel"} {
        set program_path $::KV_SENTINEL_BIN
    } else {
...
proc spawn_instance {type base_port count {conf {}} {base_conf_file ""}} {
    for {set j 0} {$j < $count} {incr j} {
        set port [find_available_port $base_port $::kv_port_count]
        ...
        if {$::tls} {
            if {$::tls_module} {
                puts $cfg "loadmodule $::KV_TLS_MODULE"
            }
            ...
            set pport [find_available_port $base_port $::kv_port_count]
            puts $cfg "port $pport"
            ...
            puts $cfg [format "tls-dh-params-file %s/../../tls/kv.dh" [pwd]]
...
proc parse_options {} {
    for {set j 0} {$j < [llength $::argv]} {incr j} {
        ...
        if {$opt eq "--single"} {
            ...
            puts "--tls-module            Run tests in TLS mode with KV module."
...
proc pause_on_error {} {
    ...
    while 1 {
        ...
        if {$cmd eq {continue}} {
            break
        } elseif {$cmd eq {show-kv-logs}} {
            set count 10
            if {[lindex $argv 1] ne {}} {set count [lindex $argv 1]}
            foreach_kv_id id {
                puts "=== KV $id ===="
                puts [exec tail -$count kv_$id/log.txt]
                puts "---------------------\n"
            }
            ...
            puts "ls                     List Sentinel and KV instances."
            puts "show-sentinel-logs \[N\] Show latest N lines of logs."
            puts "show-kv-logs \[N\]    Show latest N lines of logs."
            puts "S <id> cmd ... arg     Call command in Sentinel <id>."
            puts "R <id> cmd ... arg     Call command in KV <id>."
            puts "SI <id> <field>        Show Sentinel <id> INFO <field>."
            puts "RI <id> <field>        Show KV <id> INFO <field>."
...
#     [Rn 0] info
proc Rn {n} {
    return [dict get [lindex $::kv_instances $n] link]
}
...
proc foreach_kv_id {idvar code} {
    set errcode [catch {uplevel 1 [list foreach_instance_id $::kv_instances $idvar $code]} result]
    return -code $errcode $result
}
...
# slaves.
proc create_kv_master_slave_cluster n {
    foreach_kv_id id {
        if {$id == 0} {
            # Our master.
            R $id slaveof no one
            ...
            R $id slaveof [get_instance_attrib kv 0 host] \
                          [get_instance_attrib kv 0 port]
        } else {
...
proc kv_deferring_client {type id} {
    set port [get_instance_attrib $type $id port]
    set host [get_instance_attrib $type $id host]
    set client [kv $host $port 1 $::tls]
    return $client
}

proc kv_deferring_client_by_addr {host port} {
    set client [kv $host $port 1 $::tls]
    return $client
}

proc kv_client {type id} {
    set port [get_instance_attrib $type $id port]
    set host [get_instance_attrib $type $id host]
    set client [kv $host $port 0 $::tls]
    return $client
}

proc kv_client_by_addr {host port} {
    set client [kv $host $port 0 $::tls]
    return $client
}
