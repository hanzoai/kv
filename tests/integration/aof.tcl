...
tags {"aof external:skip logreqres:skip"} {
    ...
    start_server_aof [list dir $server_path aof-load-truncated yes] {
        ...
        test "Truncated AOF loaded: we expect foo to be equal to 5" {
            set client [kv [srv host] [srv port] 0 $::tls]
    ...
    ## Test that kv-check-aof indeed sees this AOF is not valid
    test "Short read: Utility should confirm the AOF is not valid" {
        catch {
            exec $::KV_CHECK_AOF_BIN $aof_manifest_file
        } result
    ...
    test "Short read: Utility should be able to fix the AOF" {
        set result [exec $::KV_CHECK_AOF_BIN --fix $aof_manifest_file << "y\n"]
    ...
    start_server_aof [list dir $server_path aof-load-truncated no] {
        test "AOF+LMPOP/BLMPOP: pop elements from the list" {
            set client [kv [srv host] [srv port] 0 $::tls]
            set client2 [kv [srv host] [srv port] 1 $::tls]
            wait_done_loading $client
    ...
    start_server_aof [list dir $server_path] {
        test {Successfully load AOF which has timestamp annotations inside} {
            set c [kv [srv host] [srv port] 0 $::tls]
            wait_done_loading $c
    ...
    test {Truncate AOF to specific timestamp} {
        ...
        exec $::KV_CHECK_AOF_BIN --truncate-to-timestamp 1628217473 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [kv [srv host] [srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        exec $::KV_CHECK_AOF_BIN --truncate-to-timestamp 1628217471 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [kv [srv host] [srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        exec $::KV_CHECK_AOF_BIN --truncate-to-timestamp 1628217470 $aof_manifest_file
        start_server_aof [list dir $server_path] {
            set c [kv [srv host] [srv port] 0 $::tls]
            wait_done_loading $c
            assert_equal "bar1" [$c get foo1]
        ...
        catch {exec $::KV_CHECK_AOF_BIN --truncate-to-timestamp 1628217469 $aof_manifest_file} e
    ...
    test {Test kv-check-aof for old style resp AOF} {
        create_aof $aof_dirpath $aof_file {
            append_to_aof [formatCommand set foo hello]
            append_to_aof [formatCommand set bar world]
        }
            ...
            exec $::KV_CHECK_AOF_BIN $aof_file
        } result
        assert_match "*Start checking Old-Style AOF*is valid*" $result
    }

    test {Test kv-check-aof for old style resp AOF - has data in the same format as manifest} {
        create_aof $aof_dirpath $aof_file {
    ...
    test {Test kv-check-aof for old style rdb-preamble AOF} {
        catch {
            exec $::KV_CHECK_AOF_BIN tests/assets/rdb-preamble.aof
        } result
        assert_match "*Start checking Old-Style AOF*RDB preamble is OK, proceeding with AOF tail*is valid*" $result
    }

    test {Test kv-check-aof for Multi Part AOF with resp AOF base} {
    ...
    test {Test kv-check-aof for Multi Part AOF with rdb-preamble AOF base} {
    ...
    test {Test kv-check-aof for Multi Part AOF contains a format error} {
        create_aof_manifest $aof_dirpath $aof_manifest_file {
            append_to_manifest "file appendonly.aof.1.base.aof seq 1 type b\n"
            append_to_manifest "file appendonly.aof.1.incr.aof seq 1 type i\n"
            append_to_manifest "!!!\n"
    ...
    test {Test kv-check-aof only truncates the last file for Multi Part AOF in fix mode} {
        create_aof $aof_dirpath $aof_base_file {
            append_to_aof [formatCommand set foo hello]
            append_to_aof [formatCommand multi]
            append_to_aof [formatCommand set bar world]
        ...
        assert_match "*not valid*" $result
            ...
            exec $::KV_CHECK_AOF_BIN --fix $aof_manifest_file
        } result
        assert_match "*Failed to truncate AOF*because it is not the last file*" $result
    }

    test {Test kv-check-aof only truncates the last file for Multi Part AOF in truncate-to-timestamp mode} {
        create_aof $aof_dirpath $aof_base_file {
            append_to_aof "#TS:1628217470\r\n"
            append_to_aof [formatCommand set foo1 bar1]
            append_to_aof "#TS:1628217471\r\n"
