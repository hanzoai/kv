...
set numkeys 50000
set numops 200000
set start_node_port [get_instance_attrib kv 0 port]
set cluster [kv_cluster 127.0.0.1:$start_node_port]
if {$::tls} {
    # setup a non-TLS cluster client to the TLS cluster
    set plaintext_port [get_instance_attrib kv 0 plaintext-port]
    ...
    for {set j 0} {$j < $numops} {incr j} {
        ...
        if {$j >= $numops/2 && $tribpid eq {}} {
            ...
            set tribpid [lindex [exec \
                $::KV_CLI_BIN --cluster reshard \
                127.0.0.1:[get_instance_attrib kv 0 port] \
                --cluster-from all \
                --cluster-to $target \
                --cluster-slots 100 \
                --cluster-yes \
...
test "Verify slaves consistency" {
    ...
    foreach_kv_id id {
        ...
        set masterport [get_instance_attrib kv $id port]
