/**
 * fuzzer_client.c - Client implementation for KV fuzzer
 *
 * This file implements a client that connects to a KV server,
...
static int isServerDisconnected(kvContext *ctx) {
    return (ctx->err == KV_ERR_EOF ||
            (ctx->err == KV_ERR_IO &&
             (errno == ECONNRESET || errno == EPIPE || errno == ENOTCONN)));
}

/* Connect to a KV server with optional TLS support
 * Returns a kvContext on success, NULL on error
 * ssl_config can be NULL to disable TLS */
static kvContext *connectToServer(const char *host, int port, cliSSLconfig *ssl_config) {
    kvContext *ctx;
    kvReply *reply;
    ...
    ctx = kvConnectWithOptions(&options);
    if (!ctx) {
        logMessage(LOG_ERROR, "[Thread %d] Connection error: can't allocate kv context", thread_id);
        return NULL;
    }
    ...
    kvSetTimeout(ctx, command_timeout);

    /* Establish TLS connection if ssl_config is provided */
    if (ssl_config) {
        const char *tls_err = NULL;
        if (cliSecureConnection(ctx, *ssl_config, &tls_err) == KV_ERR && tls_err) {
 ...
 * ssl_config can be NULL to disable TLS */
static kvContext *reconnectWithBackoff(const char *host, int port, cliSSLconfig *ssl_config) {
    int attempts = 0;
    kvContext *ctx = NULL;
...
static void handleMalformedReply(kvContext *ctx, FuzzerCommand *cmd) {
...
static CommandResult handleCommandError(kvContext *ctx, FuzzerCommand *cmd, time_t start_time) {
    CommandResult result = CMD_ERROR;

    if (ctx->err == KV_ERR_IO && (errno == EAGAIN || errno == EWOULDBLOCK || errno == ETIMEDOUT)) {
    ...
    } else if (ctx->err == KV_ERR_PROTOCOL) {
...
static void logReplyDebug(const char *command, kvReply *reply) {
    if (current_log_level < LOG_DEBUG) return;
...
static void validateReplyAndUpdateCounters(kvReply *reply, FuzzerCommand *cmd) {
...
static CommandResult sendCommandAndGetReply(kvContext *ctx) {
    int ret = CMD_SUCCESS;
    ...
    kvReply *reply = kvCommandArgv(ctx, cmd->argc, (const char **)cmd->argv, NULL);
...
/* Connect and fuzz a KV server with optional TLS support
 ...
 * This function is called by kv-benchmark when fuzz mode is enabled */
