/* Some unit tests that don't require KV to be running. */
...
void test_kv_parse_error_nonresp(void) {
...
void test_kv_parse_cmd_get(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "GET foo");
...
void test_kv_parse_cmd_mset(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "MSET foo val1 bar val2");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_MSG(c->result == CMD_PARSE_OK, "Parse not OK");
...
void test_kv_parse_cmd_eval_1(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "EVAL dummyscript 1 foo");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_MSG(c->result == CMD_PARSE_OK, "Parse not OK");
...
void test_kv_parse_cmd_eval_0(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "EVAL dummyscript 0 foo");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_MSG(c->result == CMD_PARSE_OK, "Parse not OK");
...
void test_kv_parse_cmd_xgroup_no_subcommand(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "XGROUP");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_MSG(c->result == CMD_PARSE_ERROR, "Unexpected parse success");
...
void test_kv_parse_cmd_xgroup_destroy_no_key(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "xgroup destroy");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
...
void test_kv_parse_cmd_xgroup_destroy_ok(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "xgroup destroy mystream mygroup");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
...
void test_kv_parse_cmd_xreadgroup_ok(void) {
    struct cmd *c = command_get();
...
void test_kv_parse_cmd_xread_ok(void) {
    struct cmd *c = command_get();
    int len = kvFormatCommand(
...
void test_kv_parse_cmd_restore_ok(void) {
    ...
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "restore k 0 xxx");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_KEY(c, "k");
...
void test_kv_parse_cmd_restore_asking_ok(void) {
    /* The ordering of RESTORE and RESTORE-ASKING in the lookup-table was wrong
     * in a previous version, leading to the command not being found. */
    struct cmd *c = command_get();
    int len = kvFormatCommand(&c->cmd, "restore-asking k 0 xxx");
    ASSERT_MSG(len >= 0, "Format command error");
    c->clen = len;
    kv_parse_cmd(c);
    ASSERT_KEY(c, "k");
...
int main(void) {
    ...
    test_kv_parse_cmd_mset();
    test_kv_parse_cmd_eval_1();
    test_kv_parse_cmd_eval_0();
    test_kv_parse_cmd_xgroup_no_subcommand();
    test_kv_parse_cmd_xgroup_destroy_no_key();
    test_kv_parse_cmd_xgroup_destroy_ok();
    test_kv_parse_cmd_xreadgroup_ok();
    test_kv_parse_cmd_xread_ok();
    test_kv_parse_cmd_restore_ok();
    test_kv_parse_cmd_restore_asking_ok();
    test_kv_parse_cmd_georadius_ro_ok();
    return 0;
}
