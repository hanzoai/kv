...
#
# KV has the config `cluster-preferred-endpoint-type` which
# controls how the endpoint is returned in ASK/MOVED redirects and in
# CLUSTER SLOTS. This testcase verifies correct handling when KV
# announce a hostname 'localhost' instead of an IP.
#
# KV also provides additional metadata in the CLUSTER SLOTS response
...
timeout 5s ./simulated-kv.pl -p 7402 -d --sigcont $syncpid2 <<'EOF' &
EXPECT CONNECT
EXPECT ["ASKING"]
SEND +OK
EXPECT ["GET", "foo"]
